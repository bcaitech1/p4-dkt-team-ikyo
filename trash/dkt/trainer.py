...
def run(args, train_data, valid_data):
    ...
    for epoch in range(args.n_epochs):
        ...
        ### TRAIN
        train_auc, train_acc, train_loss, total_preds, total_targets = train(train_loader, model, optimizer, args, epoch, last_whole_y_t, last_whole_y_pred, epoch_gamma)
        
        ### VALID
        auc, acc, _ , _ = validate(valid_loader, model, args)

        if args.loss == 'roc':
            last_whole_y_t = torch.tensor(total_targets).cuda()
            last_whole_y_pred = torch.tensor(total_preds).cuda()
...
def train(train_loader, model, optimizer, args, epoch, last_whole_y_t, last_whole_y_pred, epoch_gamma):
    model.train()
    ...
    for step, batch in enumerate(train_loader):
        ...
        if args.loss == 'bce':
            ...
            if epoch > 0:
                loss = roc_star_loss(targets, preds, epoch_gamma, last_whole_y_t, last_whole_y_pred)
            else:
                loss = compute_loss(preds, targets)
    ...
    return auc, acc, loss_avg, total_preds, total_targets
